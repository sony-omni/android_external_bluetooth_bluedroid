    ...
    while (TRUE)
    {
        ...
        if (service & BTA_HFP_SERVICE_MASK)
        {
            if ((p_attr = SDP_FindAttributeInRec(p_rec, ATTR_ID_SUPPORTED_FEATURES)) != NULL)
            {
                ...
                APPL_TRACE_DEBUG("bta_ag_sdp_find_attr: peer_attr 0x%x",  p_attr->attr_value.v.u16);
                APPL_TRACE_WARNING("bta_ag_sdp_find_attr not using value as p_scb->peer_features");
